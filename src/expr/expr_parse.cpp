	...
	, msgs{}
{
	...
	token_cache.tokens.v = PushArray(arena, Token, 64);
...
ParseNumeric(Parser* parser)
{
	...
	result->flags = ComputeNodeFlags(result);
...
IsInteger(Node* expr)
{
	return (expr->kind == NodeKind::Number) ? 1 : 0;
}

internal NodeFlags 
ComputeNodeFlags(Node* node)
{
	NodeFlags flags{};
	switch (node->kind)
	{
		...
		{
			flags = NodeFlags::Integer_B;
		}
		case NodeKind::Variable:
		{
			flags = NodeFlags::Symbol_B;	
		}
		case NodeKind::UnaryOp:
		{
			if (node->un_ops == UnOpKind::Negate)
				flags = NodeFlags::DiffOp_B | NodeFlags::UnaryDiffOp_S;
			else if (node->un_ops == UnOpKind::Positive)
				flags = NodeFlags::SumOp_B | NodeFlags::UnarySumOp_S;
		}
		case NodeKind::BinaryOp:
		{
			if (node->bin_ops == BinOpKind::Minus)
				flags = NodeFlags::DiffOp_B | NodeFlags::BinaryDiffOp_S;
			else if (node->bin_ops == BinOpKind::Divide)
			{
				if (IsInteger(node->bin_left) && IsInteger(node->bin_right))
					flags = NodeFlags::FracOp_B;
				else
					flags = NodeFlags::QuotOp_B;
			}
		}
		case NodeKind::NaryOp:
		{
			if (node->nary_ops == NaryOpKind::Plus)
				flags = NodeFlags::SumOp_B;
			else if (node->nary_ops == NaryOpKind::Multiply)
				flags = NodeFlags::ProdOp_B;
		}
